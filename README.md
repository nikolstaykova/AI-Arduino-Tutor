...
- **One world per group of Arduino's official [built-in examples](https://docs.arduino.cc/built-in-examples/)** — Basics, Digital, Analog, Communication, Control Structures, Sensors, Display, USB, Arduino ISP — plus an **AI Lab** for lessons you invent. Every example you build with real parts is a level: **48 levels**, all playable, on the Uno, the Mega (Analog Write) and the Leonardo (the USB keyboard/mouse levels); the Arduino ISP world programs a bare ATmega328P chip and then runs it on the breadboard. Code-only examples (Strings, pure Serial sketches) aren't on the map. The levels are written with the **lesson kit** (`tools/lesson_kit.py` + `tools/lessons_spec.py`: circuit blocks lay out the breadboard and write the beginner steps around the official sketch), and every lesson must pass the same validator as Claude's.
- **Sparky drives a little car** along one winding road through themed low-poly worlds (a garden with a windmill, a neon arcade, a desert with a radio tower, a lighthouse bay, a switchyard, a forest, a pixel city with robots, a keyboard cove, a chip factory) and a countryside of hills, rivers, lakes, cottages, sheep, bunnies and LED mushrooms.
...
- **True-size 3D models** (three.js, millimetres on the 0.1" pitch): a breadboard where **every hole is its own object**, an Arduino Uno with every component labelled and explained (USB, regulator, ATmega328P, crystal, headers …), and a 3D model for every one of the ~110 parts and tools in the library.
...
### Accounts and shared lessons (hosted)
- **Sign in with Google**; your stars, XP and progress follow your account. Every lesson anyone creates is shared — if Mert makes one, it's on your map too, "made by Mert". Set up once with Supabase's free plan: [`cloud/SETUP.md`](./cloud/SETUP.md). Without it, the app runs on local files.
- **"This might be the same"** — before spending Claude on a new lesson, it checks the tutorial link (every lesson records the tutorial it follows) and the words against every existing lesson. You can **Open level** (Sparky drives there) or create it anyway.
