...
# ---- Arduino as ISP: program a bare ATmega328P chip -----------------------------------------------------
ARDUINO_ISP_CODE = r"""
// ArduinoISP
...
// Configure which pins to use (CircuitQuest: the Uno wiring from the tutorial;
// the official file also has variants for other boards, left out here):
#define RESET 10  // Use pin 10 to reset the target rather than SS
#define LED_HB 9
#define LED_ERR 8
#define LED_PMODE 7

// On an Uno the hardware SPI pins are 11 (MOSI), 12 (MISO) and 13 (SCK):
...
def arduino_isp():
    L = Lesson("arduino-isp", "Arduino as ISP", "Turn your Arduino into a programmer and use it to put a bootloader on a bare ATmega328P chip.",
               D + "arduino-isp/ArduinoISP/", "Arduino as ISP", requires=["blink"], full_board=True, fixed_pins=True, code=ARDUINO_ISP_CODE)
    L.bare_chip()
    L.isp_wires()
    L.reset_cap()
    L.led(9, "green", "the green LED")      # heartbeat: the programmer is running
    L.led(8, "red", "the red LED")          # error
    L.led(7, "yellow", "the yellow LED")    # programming
    L.upload("", clip=(
        "Program the chip in four steps:<br>"
        "1. Pull the 10 µF capacitor out for a moment. Open <b>File › Examples › 11.ArduinoISP › ArduinoISP</b> (the same code as here) and click Upload. "
        "Put the capacitor back.<br>"
        "2. <b>Tools › Board</b>: Arduino Uno. <b>Tools › Programmer</b>: Arduino as ISP.<br>"
        "3. <b>Tools › Burn Bootloader</b>. The green LED breathes slowly while the programmer runs, the yellow one flickers while it writes, and the red one lights if something goes wrong.<br>"
        "4. Open <b>File › Examples › 01.Basics › Blink</b> and choose <b>Sketch › Upload Using Programmer</b>: the chip now has Blink. "
        "Keep it for the next level."))
    return L


def arduino_to_breadboard():
    L = Lesson("arduino-to-breadboard", "Chip on a Breadboard", "Run a sketch on a bare ATmega328P chip — a whole Arduino made from a chip and four small parts.",
               D + "arduino-isp/ArduinoToBreadboard/", "Arduino to Breadboard", requires=["arduino-isp"], full_board=True, fixed_pins=True, code="""
/*
...
""")
    L.col = 6                            # room on the left for the reset resistor
    L.bare_chip()
    L.runs_on = L.chip
    L.reset_pullup()
    L.chip_led("19")
    L.upload("", clip=("Nothing to upload: the chip already has Blink from the last level. Plug in the USB cable — the Uno only gives the chip power now. "
                       "The LED on chip pin 19 blinks once a second. (A fresh chip? Program it first, as in the last level.)"))
    return L


LESSONS += [arduino_isp, arduino_to_breadboard]
